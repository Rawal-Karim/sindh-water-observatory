...
$('clearArea').onclick=()=>{stopDrawing();selectedBox=null;if(selectionLayer)map.removeLayer(selectionLayer);if(selectionGoogle)selectionGoogle.setMap(null);$('areaStatus').textContent='Click Draw area to select a rectangle.';$('exportArea').disabled=true;calendarAreaChanged();};
...
async function sceneState(item,b,insideSindh){const scl=item.assets?.scl,epsg=Number(item.properties['proj:epsg']);if(!scl?.['proj:transform']||!(epsg>32600&&epsg<32661))return'unverified';
 const t=scl['proj:transform'],[H,W]=scl['proj:shape'],c=[[b[0],b[1]],[b[0],b[3]],[b[2],b[1]],[b[2],b[3]]].map(p=>utm(p[0],p[1],epsg-32600)),xs=c.map(p=>p[0]),ys=c.map(p=>p[1]);
 const c0=Math.floor((Math.min(...xs)-t[2])/t[0]),c1=Math.ceil((Math.max(...xs)-t[2])/t[0]),r0=Math.floor((Math.max(...ys)-t[5])/t[4]),r1=Math.ceil((Math.min(...ys)-t[5])/t[4]);
 if(c0<0||r0<0||c1>W||r1>H)return'unverified';// area crosses this tile's edge
...
// That day's true-colour photo sits under the Detected water layer only. The Reconstructed surface keeps the
// basemap on land by design, so the (coarser) photo must not cover it there. The opacity slider fades both.
